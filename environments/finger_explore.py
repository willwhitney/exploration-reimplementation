...
@SUITE.add('exploration')
def turn_easy_narrow(time_limit=_DEFAULT_TIME_LIMIT,
...
@SUITE.add('exploration')
def turn_hard_narrow(time_limit=_DEFAULT_TIME_LIMIT,
                     random=None, environment_kwargs=None):
...
class Turn(base.Task):
    ...
    def initialize_episode(self, physics):
        target_angle = self.random.uniform(-np.pi / 4, np.pi / 4)
        ...
        _set_narrow_joint_angles(physics, self.random)
...
def _set_narrow_joint_angles(physics, random, max_attempts=1000):
    """Sets the joints to a random collision-free state."""

    span = np.pi / 4
    for _ in range(max_attempts):
        physics.named.data.qpos['proximal'] = np.pi + random.uniform(-span, span)
        physics.named.data.qpos['distal'] = random.uniform(-span, span)
        physics.named.data.qpos['hinge'] = np.pi + random.uniform(-span, span)
