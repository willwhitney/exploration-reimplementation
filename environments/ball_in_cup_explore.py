...
def get_model_and_assets():
  ...
  return (resources.GetResource(f'{current_dir}/ball_in_cup_explore.xml'),
          common.ASSETS)

@SUITE.add('exploration')
def catch(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns the Ball-in-Cup Explore task."""
  physics = Physics.from_xml_string(*get_model_and_assets())
  task = BallInCupExplore(random=random)
...
class Physics(mujoco.Physics):
  """Physics with additional features for the Ball-in-Cup Explore domain."""
...
class BallInCupExplore(base.Task):
  """The Ball-in-Cup Explore task. Put the ball in the cup."""
  ...
  def initialize_episode(self, physics):
    ...
    while penetrating:
      ...
      physics.named.data.qpos['ball_x'] = self.random.uniform(-.1, .1)
      physics.named.data.qpos['ball_z'] = self.random.uniform(0.2, .3)
