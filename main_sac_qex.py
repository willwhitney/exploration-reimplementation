...
# - novelty Q lives inside SAC and is updated from a replay
# - there is no optimism
# - the policy is trained to maximize Q_task + Q_explore
...
class AgentState():
    ...
    bonus_replay: Any = struct.field(pytree_node=False)
    j_action_spec: Any
    n_candidates: int
...
def run_episode(agent_state: AgentState, rng, env,
    ...
    while not timestep.last():
        ...
        if len(agent_state.replay) < warmup_steps:
        ...
        if train:
            transition = (s, a, sp, r)
            bonus_transition = (s, a, sp, novelty_reward)
            rng, update_rng = random.split(rng)

            # add transition with the novelty reward to the bonus replay
            agent_state.bonus_replay.append(*bonus_transition)

            # add transition with real reward to regular replay
...
def display_state(agent_state: AgentState, ospec, aspec,
    ...
    assert (policy.__name__ == 'policies.sac_qex_policy')
    import torch
    ...
    def get_exploration_value(s, a):
        s = torch.FloatTensor(np.array(s)).to(policy_state.device)
        a = torch.FloatTensor(np.array(a)).to(policy_state.device)
        with torch.no_grad():
            v = policy_state.exploration_critic.Q1(torch.cat([s, a], dim=-1))
        return v.cpu().detach().numpy()
    exploration_q_map = render_function(
        get_exploration_value,
        agent_state.bonus_replay,
        ospec, aspec, bins=bins)
    subfigs.append((exploration_q_map, "Explore value (max)"))
...
def main(args):
    ...
    bonus_replay = replay_buffer.Replay(state_shape, action_shape)
    ...
    for episode in range(1, args.max_episodes + 1):
        ...
        for _ in range(int(args.max_steps * args.policy_updates_per_step)):
            ...
            bonus_transitions = agent_state.bonus_replay.sample(1024)
            bonus_transitions = tuple((jnp.array(el) for el in bonus_transitions))
            policy_state = policy.update_fn(
                policy_state, transitions, bonus_transitions)
        agent_state = agent_state.replace(policy_state=policy_state)
...
if __name__ == '__main__':
    ...
    # policy settings
    parser.add_argument('--policy', type=str, default='sac_qex')
    ...
    args.save_dir = f"results/sac_qex/{args.name}"
    ...
    assert (args.policy == 'sac_qex')
    import policies.sac_qex_policy as policy
