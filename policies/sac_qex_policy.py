...
from policies.sac_qex.agent import sac_qex


def init_fn(state_spec, action_spec, seed, bonus_scale, **kwargs):
    ...
    sac_args['bonus_scale'] = bonus_scale  # // 2
    return sac_qex.SACAgent(**sac_args)


def action_fn(sac_agent: sac_qex.SACAgent, state, n=1, explore=True):
...
def update_fn(sac_agent: sac_qex.SACAgent, transitions, bonus_transitions):
    transitions = (np.array(jax_array) for jax_array in transitions)
    bonus_transitions = (np.array(jax_array) for jax_array in bonus_transitions)
    sac_agent.update(transitions, bonus_transitions)
    return sac_agent
