...
@struct.dataclass
class PolicyFns():
    action_fn: types.FunctionType
    update_fn: types.FunctionType


def make_policy(state_spec, action_spec, seed, n_candidates=32, **kwargs):
    ...
    def update_fn(policy_state: PolicyState, transitions):
        ...
        policy_rng, candidate_rng = random.split(policy_state.rng)
        candidate_actions = utils.sample_uniform_actions(
            action_spec, candidate_rng, n_candidates * bsize)

        candidate_shape = (bsize, n_candidates, *candidate_actions.shape[1:])
        candidate_actions = candidate_actions.reshape(candidate_shape)
    ...
    return init_fn(), PolicyFns(action_fn, update_fn)
